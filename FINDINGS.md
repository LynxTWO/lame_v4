...
3. *The real lever - fix the objective.* `outer_loop` compares candidate quantizations with
   ...
   sum of per-band log noise-to-mask. We swept objectives x search depth (via a temporary
...
- Cost: **~15x default `-q0`** (Tom's Diner CBR128: 22.3 s vs 1.5 s; CBR320 39.9 s;
...
(**1.54x**), ABR192 29.8 -> 19.8 s (**1.50x**), CBR128 22.3 -> 19.0 s (1.17x - joint stereo
gives the mid channel most of the search work at low bitrates, and the per-granule join waits
for the slower channel), default `-q0` 1.5 -> 1.4 s (1.06x - quantization is a smaller share
...
| full Pareto dominance (`over_count` AND `over_noise` AND `tot_noise` AND bits, >=1 strict) | **+0.45 dB** | swaps that *every internal metric calls unambiguous upgrades* still measure worse on the independent meter |
| **dual referee**: strictly better by an independent in-loop Bark-spread mask (the external meter's model rebuilt in the MDCT domain) AND tot_noise <= AND bits <= | **+0.20 dB** | even swaps endorsed by two masking models at no bit cost leak quality through channels invisible at selection time |
...
reference: ~3x v2 (66 s vs 22 s on the ABX track at CBR128), threads-identical.
...
tuning 0.5-3.5, short-block thresholds x0.5-2), 128 random configs + 2 coordinate-refinement
...
(no individual training file may exceed its own stock audNMR by >0.1 dB, lambda=10). The search
...
loop, transient-HF terms in the fitness, tol=0/lambda=100 to see if *any* mean gain survives a
...
- [x] Podcast constrained-optimizer v2 (`tools/podcast/`, design + implementation notes in
      `docs/podcast-optimizer-design.md`): every flavor (effort x filter over ABR/CBR/true-VBR)
...
- [x] **Finding 4: bit-exact channel-parallel quantization** (`--threads 2`) - gate passes
      70/70 with threads on AND off; **1.54x on `--quality-max` CBR320, 1.50x ABR192** (real
...
- [x] Analysis-front (psymodel/MDCT) parallelism audit (`docs/analysis-front-audit.md`) - 
      channel-parallel is feasible but worth only ~1.2x at default settings; ABR worker
      dispatch frame-batched as the audit's follow-up (1.56x, hash-identical).
...
- [ ] Campaign 6 options (in order of promise): validation-split veto inside the search;
      transient-HF fitness terms; hard audibility ceiling (tol=0, lambda=100); or bank the
