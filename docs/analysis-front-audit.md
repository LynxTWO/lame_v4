...
1.5× on `--quality-max` CBR320/ABR192). The analysis front (psymodel + MDCT ≈ 39%) is the
...
- **M/S FFT is derived in place from L/R** (`vbrpsy_compute_fft_l`, chn==2 branch): the L and
  R FFT work buffers `wsamp_L[0..1]` are *overwritten* with (L±R)/√2. Consequences: L and R
...
1. **Channel-parallel psymodel within a granule** (bit-exact candidate): run L and R analysis
   ...
   Finding 4. Parallelizable share ≈ ¾ of psymodel → psy 26% → ~15% of total: **~1.13×** at
   default settings (~nothing at `--quality-max`). Roughly six fork/join points per granule.
2. **+ channel-parallel MDCT**: 13% → ~7%: combined **~1.2×** at default.
3. **Frame pipelining** (analysis of frame N+1 concurrent with quantization of frame N):
   theoretical ~1.6× at default when combined with (1)+(2) and threaded quantization, but
...
4. **Do nothing here; deepen the search instead**: quality-max v3 (wider search) concentrates
   ...
   ~100× realtime single-threaded; per-file speed there is nobody's bottleneck (batch
