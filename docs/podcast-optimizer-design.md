...
`--quality-max`** (see the q-note below); lowpass in {auto,16,17,18,19 kHz}; highpass in
{off,40,60,80 Hz}; optional fractional-`-V` probes landing near target.

Mono (96): mode = mono; ABR 96 plus nearby means; q sweep; lowpass in {auto,12,14,15.5,16 kHz};
highpass in {off,50,70,90 Hz}; a 32 kHz sample-rate candidate for speech-heavy material;
fractional-`-V` probes.
...
- **v2 architecture - land every flavor, then compare.** v1 used a fixed grid of requests and
  ...
  0.5-wide window between steps. v2 instead gives each *flavor* (mode x effort x filter over
